...
# out1 <- median_cl_boot(x = mtcars$wt)
# out2 <- median_cl_boot(x = mtcars$wt, conf=0.99)
...
#   expect_equal(median_cl_boot(mtcars$wt), expected[[1]])
#   expect_equal(median_cl_boot(x = mtcars$wt, conf = 0.99),
#                expected[[2]])
# })
